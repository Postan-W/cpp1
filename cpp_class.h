 ...
 * 可以被外部调用。
    ...
    11.初始值列表有时候是必要的。因为在构造函数体开始执行之前，初始化工作就完成了,对于一般类型的数据成员来说，我们或许可以在
    构造函数中通过显式地赋值对其进行内容的更改，但对于const成员或者引用类型的成员来讲，它们要么是类内初始值，要么是默认值(引用类型
    的话就直接报错，因为其必须被初始化)，而随着构造函数体开始一开始执行，初始化就完成了，const性质也被确定。所以我们显式地主动初始
    这种成员的唯一机会就是通过构造函数初始值列表。
